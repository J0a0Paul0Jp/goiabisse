...
bool Market::append( const Order& order )
{
  if ( order.getSide() == Order::buy )
    bidOrders.insert( BidOrders::value_type( order.getPrice(), order ) );
  else
    askOrders.insert( AskOrders::value_type( order.getPrice(), order ) );
  return true;
}

void Market::remove( const Order& order )
{
  std::string id = order.getClientID();
  if ( order.getSide() == Order::buy )
  {
    BidOrders::iterator i;
    for ( i = bidOrders.begin(); i != bidOrders.end(); ++i )
  ...
  else if ( order.getSide() == Order::sell )
  {
    ...
    for ( i = askOrders.begin(); i != askOrders.end(); ++i )
      if ( i->second.getClientID() == id )
      {
        askOrders.erase( i );
        return ;
...
bool Market::match( std::queue < Order > & orders )
{
  while ( true )
  {
    if ( !bidOrders.size() || !askOrders.size() )
      return orders.size() != 0;

    BidOrders::iterator iBid = bidOrders.begin();
    AskOrders::iterator iAsk = askOrders.begin();
    ...
    if ( iBid->second.getPrice() >= iAsk->second.getPrice() )
    {
      ...
      if ( bid.isClosed() ) bidOrders.erase( iBid );
      if ( ask.isClosed() ) askOrders.erase( iAsk );
    }
    else
      return orders.size() != 0;
  }
...
void Market::print() const
{
  ...
  for ( iBid = bidOrders.begin(); iBid != bidOrders.end(); ++iBid )
    std::cout << iBid->second << std::endl;

  std::cout << std::endl << std::endl;

  std::cout << "OFERTAS:" << std::endl;
  std::cout << "-----" << std::endl << std::endl;
  for ( iAsk = askOrders.begin(); iAsk != askOrders.end(); ++iAsk )
